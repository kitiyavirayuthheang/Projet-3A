...
\begin{frame}
On considère \textbf{un modèle de marché financier} sur l'espace de probabilité (\Omega, (\mathcal{F}_t, t \in [0,T]), \mathbb{P})$. Les prix des actions (on considère d actions) évoluent selon l'équation différentielle stochastique linéaire : 
\begin{center}
\displaystyle S^i_t = S^i_0 + \int_{0}^{t} S^i_s b^i_s ds +  \int_{0}^{t} S^i_s \sigma^i_s dW_s, 0 \leq t \leq T, S_0 \in \mathbb{R}^d, i = 1, ... , d
\end{center}
